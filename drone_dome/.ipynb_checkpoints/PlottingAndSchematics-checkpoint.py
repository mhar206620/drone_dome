...
class PlottingAndSchematics():
    ...
    def diagnostics(self, ring_dataframe, arch_dataframe):
        ...
        expect the left hand column to look exactly like the right hand column, but rotated 90 degrees
            ...
            user shouldn't ever need to run this method unless something wonky happens
            but useful to have for troubleshooting
        '''
...
#         PLOT_BG = cimgt.MapboxStyleTiles(MAPBOX_KEY, MAPBOX_USERNAME, MAPBOX_STYLE)
        ...
        ax.set_extent([ring_dataframe.longitude.min()-BORDER, ring_dataframe.longitude.max()+BORDER, ring_dataframe.latitude.min()-BORDER, ring_dataframe.latitude.max()+BORDER], crs=ccrs.Geodetic())
        ...
        xlim = ax.get_xlim()
        ylim = ax.get_ylim()
        ax.fill((xlim[0],xlim[0],xlim[1],xlim[1],xlim[0]), (ylim[0],ylim[1],ylim[1],ylim[0],ylim[0]), color=(0.7,0.7,0.7,0.4)) #color is rgb, alpha
